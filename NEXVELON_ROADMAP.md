...
**🚧 IN PROGRESS as of Session W (2026-05-12):** Pass 8 of 11 complete. `NEXVELON_PERMISSIONS_DESIGN.md` v0.8. Pass 1-7 condensed to summary sections. Pass 8 (Permissions Editor UI) full content §15-§27. Workspace architecture (single page with six sections; not separate tabs). Six sections fully specified: Actions matrix with 4-tier hierarchy + bulk edit + comparison view, Field Visibility matrix with mask previews + audit-on-read indicators, Data Scopes role × resource matrix with impact preview, Overrides with Active + Pending Requests + History sub-tabs + direct grant, Custom Roles clone-from-existing wizard + archival, Audit Log embedded query interface with row detail + related events chain + export. Persistent header with global search + save state indicator. Cross-section linking. Transactional save flow with conflict detection (cascading effects, SoD violations, regulatory conflicts). Undo/redo. Mobile responsive. WCAG 2.1 AA accessibility. Performance budgets <1.5s initial / <500ms small save / <30ms search. Two architectural decisions: workspace pattern (not tabs); transactional save (not optimistic). 8 Pass 8 open questions resolved. 2 new audit event types added (32 total now). Migration order extended +6 steps (now 45 total). Pending: Pass 9 (Effective-permissions caching), Pass 10 (Cross-cutting enforcement), Pass 11 (Migration plan).

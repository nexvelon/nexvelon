...
**🚧 IN PROGRESS as of Session U (2026-05-12):** Pass 6 of 11 complete. `NEXVELON_PERMISSIONS_DESIGN.md` v0.6. Pass 1-5 condensed to summary sections (full content in git history at noted commits). Pass 6 (Append-Only Audit Pattern) full content §13-§23. Uniform append-only ledger pattern across 8 ledgers (permission_audit_log + 7 module ledgers from §0.4 #10). Time-based monthly partitioning at v1; PostgreSQL UPDATE/DELETE blocking triggers; 21 enumerated event types with JSON payload schemas; reversal pattern (insert offsets); 3 first-class API endpoints + M13 reports integration + audit_log_combined UNION view; cold archival + hash-chain tamper-evidence deferred to Phase 2; performance <2ms insert / <200ms entity history / <2s cross-ledger; volume ~38M rows/year combined. Three architectural decisions: many tables one pattern, monthly partitioning, 3-layer query (API + reports + SQL view). 7 Pass 6 open questions resolved. Migration order extended (+9 steps; now 32 total). Pending: Pass 7 (Request-admin-access workflow), Pass 8 (Permissions editor UI), Pass 9 (Effective-permissions caching), Pass 10 (Cross-cutting enforcement), Pass 11 (Migration plan).

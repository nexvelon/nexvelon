...
**🚧 IN PROGRESS as of Session S (2026-05-12):** Pass 4 of 11 complete. `NEXVELON_PERMISSIONS_DESIGN.md` v0.4. Pass 1 condensed §1-§8 (full at 9008fad). Pass 2 condensed §9 (full at 1bafbd4). Pass 3 condensed §10 (full at ff08703). Pass 4 (Field-Level Visibility Engine) full content §11-§21. Backend serialization pipeline (8-stage) + frontend `<FieldGated>` wrapper architecture + 12 standard mask types library + async batched audit-on-read (100ms/50-entry/1000-backpressure/reconciliation cron/circuit breaker) + defense-in-depth Postgres views for 5 highest-sensitivity resources + complete 47-flag catalog mapped to database columns and mask types and role defaults across all 13 modules. Three architectural decisions: two-layer implementation, bulk visibility resolution, Postgres views for sensitive 5 only. Six Pass 4 open questions resolved. Pending: Pass 5 (Status surface bindings), Pass 6 (Append-only audit), Pass 7 (Request-admin-access workflow), Pass 8 (Permissions editor UI), Pass 9 (Effective-permissions caching), Pass 10 (Cross-cutting enforcement), Pass 11 (Migration plan).

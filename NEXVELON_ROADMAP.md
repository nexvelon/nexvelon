...
**🚧 IN PROGRESS as of Session T (2026-05-12):** Pass 5 of 11 complete. `NEXVELON_PERMISSIONS_DESIGN.md` v0.5. Pass 1 condensed §1-§8 (full at 9008fad). Pass 2 condensed §9 (full at 1bafbd4). Pass 3 condensed §10 (full at ff08703). Pass 4 condensed §11 (full at de1905f). Pass 5 (Status Surface Binding Layer) full content §12-§23. New schema: status_behavior_bindings (polymorphic), status_transition_definitions, effective_status_bindings_cache + permission_definitions.binding_dependencies column. 14 standard binding names across 3 categories (8 action-gating, 6 effect-triggering, 5 UI-driving). ~2000 binding rows + ~600 transition rows seed. Integration with Pass 3 A1 via Phase 3.3 (<1ms overhead; preserves <5ms total budget). State transition matrices for invoice_statuses and contractor_wo_statuses (Ontario 60-day lien clock). System-locked vs operator-configurable bindings catalogued enforcing §0.4 #8/#11/#12/#13/PCI/Construction Act/lien/append-only commitments. Effect executor with idempotent execution. Two architectural decisions: polymorphic bindings table; Phase 3 integration. Seven Pass 5 open questions resolved. Pending: Pass 6 (Append-only audit), Pass 7 (Request-admin-access workflow), Pass 8 (Permissions editor UI), Pass 9 (Effective-permissions caching), Pass 10 (Cross-cutting enforcement), Pass 11 (Migration plan).

...
**🚧 IN PROGRESS as of Session Q (2026-05-12):** Pass 2 of 11 complete. Action Vocabulary Catalog (Pass 1) + Database Schema (Pass 2) locked in `NEXVELON_PERMISSIONS_DESIGN.md` v0.2. Pass 1 normalizes ~1260 actions to `resource:verb[:qualifier]`. Pass 2 specifies 14 tables across 5 groups: core permissions (5 tables), field visibility (3 tables), data scopes (3 tables), append-only audit (1 table with UPDATE/DELETE triggers), cross-cutting constraints (3 tables — separation of duties / regulatory expiry / geolocation retention). Plus materialized view permission_resolution_view. Migration order specified. Three architectural decisions locked: one row per action, trigger-invalidated cache, orthogonal data scopes. Six Pass 2 open questions resolved. Pending: Pass 3 (Resolution algorithm), Pass 4 (Field visibility engine), Pass 5 (Status surface bindings), Pass 6 (Append-only audit), Pass 7 (Request-admin-access workflow), Pass 8 (Permissions editor UI), Pass 9 (Effective-permissions caching), Pass 10 (Cross-cutting enforcement), Pass 11 (Migration plan).

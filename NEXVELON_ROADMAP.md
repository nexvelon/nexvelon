...
**🚧 IN PROGRESS as of Session R (2026-05-12):** Pass 3 of 11 complete. `NEXVELON_PERMISSIONS_DESIGN.md` v0.3. Pass 1 (Action Vocabulary Catalog) condensed at §1-§8 (full at 9008fad). Pass 2 (Database Schema) condensed at §9 (full at 1bafbd4). Pass 3 (Resolution Algorithm) full content §10-§19. Three intertwined runtime algorithms specified: A1 (action grant — 7-phase: cache lookup → base table resolution → cross-cutting constraints → audit), A2 (data scope returning SQL filter + bind params), A3 (field visibility returning visible/masked/hidden). Performance budget <5ms p99 compound resolution; <1ms cache hit; <50ms p99 typical list endpoint. Five worked resolution traces. Five edge cases (granted-but-blocked-by-regulatory-expiry, co-sign, public actions, system-generated, expired temp overrides). RLS as defense-in-depth. Failure modes (fail-closed grants, fail-open audit). Debug API. Six observability metrics. Six Pass 3 open questions resolved. Pending: Pass 4 (Field visibility engine), Pass 5 (Status surface bindings), Pass 6 (Append-only audit), Pass 7 (Request-admin-access workflow), Pass 8 (Permissions editor UI), Pass 9 (Effective-permissions caching), Pass 10 (Cross-cutting enforcement), Pass 11 (Migration plan).

...
**🚧 IN PROGRESS as of Session V (2026-05-12):** Pass 7 of 11 complete. `NEXVELON_PERMISSIONS_DESIGN.md` v0.7. Pass 1-6 condensed to summary sections. Pass 7 (Request-Admin-Access Workflow) full content §14-§26. State machine (Pending → Approved → Granted → Expired/Revoked + Rejected/Cancelled paths). Four request types: permission_grant / field_visibility_grant / data_scope_grant / role_temporary_assignment. New `user_role_assignments` table introduced for multi-role support. Full schema for request_admin_access expanded with 30+ columns (polymorphic target, lifecycle timestamps, dual reasoning capture, forensic metadata, activity tracking). Approval workflow end-to-end (submit → notify → review → approve/reject → grant fires → notify requester). Notification routing v1 (all admins; Phase 2 routing rules placeholder). Auto-expiry 3 paths (pending timeout / duration end / one-time used). 8 edge cases handled. Integration with Pass 4 audit-on-read (3 events: grant + audit + every read). 9 new event types added to permission_audit_log catalog (30 total). Two architectural decisions: separate Approved/Granted states; all-admins notification v1 with Phase 2 routing placeholder. 7 Pass 7 open questions resolved. Migration order extended +7 steps (now 39 total). Pending: Pass 8 (Permissions editor UI), Pass 9 (Effective-permissions caching), Pass 10 (Cross-cutting enforcement), Pass 11 (Migration plan).

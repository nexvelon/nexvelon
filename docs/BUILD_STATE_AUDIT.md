...
8. **Client-facing portals are now partial.** The quote e-acceptance portal (`/q/[token]`) shipped (QUOTE-PORTAL-1 #397: immutable send snapshot + append-only acceptance, migration 0129); the invoice customer-payment portal is still unbuilt. The internal client-onboarding `app/invite/[token]/client` route also exists. *(Original finding: no client-facing portals existed at audit time.)*
...
| **Quotes (M5)** | 12 tables, 3 quote types, client portal e-acceptance, immutable send snapshots, T&C auto-composition | Full builder `app/(app)/quotes/**` (~30 builder components, PDF preview, scope/dispatch/monitoring editors); `lib/api/quotes.ts`, `quote-audit.ts`, `quote-portal.ts`; `quotes` (0027), `quote_audit_log` (0038/0040), quote labour (0055); quote→project/CO conversion (0086); **client e-acceptance portal `/q/[token]` + immutable send-snapshot + append-only acceptance tables (QUOTE-PORTAL-1 #397, migration 0129)** | 3 quote types reduced to a 2-value target-kind flag; ~3–5 tables vs 12. RESOLVED (QUOTE-PORTAL-1 #397): client portal / e-acceptance and the immutable send-snapshot table now exist. NOTE (RECUR-1 #396): `service_contracts` now exist as a first-class entity (recurring billing), but the *Service Contract quote type → contract* conversion is deferred (roadmap RECUR-2); contracts are created directly. | Medium |
...
| **Vendors (M8)** | 8 tables, T5018, onboarding gates, insurance/WSIB expiry, performance scoring w/ auto-degrade, banking encrypted | `vendors/**`, `vendors.ts`, `vendor-bills.ts`, `vendor-metrics.ts`, `purchase-orders.ts`, `t5018.ts`; `vendors` (0030), `purchase_orders` (0031), `vendor_bills` (0092); T5018 real; scorecard derived from PO receipts (0109); **banking encrypted at rest (SEC-2, `0125`/`0126`, `account_number_encrypted`)** | **Banking encryption DID ship** (SEC-2 #392 — corrected REALITY-3). Still absent: performance scoring is **read-only/derived** (no auto-degrade, no stored score/preferred-status); vendor onboarding gates; insurance/WSIB expiry lives in subs, not vendors; ~5 tables vs 8 | High |
| **Invoices / AR (M9)** | 14 tables, AR+AP, Stripe customer payment portal, 3-way match, SoD on AP, holdback | Real: `invoices/**`, `lib/api/invoices.ts` (1218 ln), `holdback.ts`, `deposits.ts`; `invoices`+`invoice_lines`+`invoice_counters` (0043), `invoice_payments` (0090), `project_deposits`+`deposit_applications` (0091), `holdback_releases` (0094); **recurring invoices auto-generated from service contracts (RECUR-1 #396, `service_contracts` 0128 + `lib/recurring/generate.ts` + daily cron)** | **No Stripe/customer payment portal**; no standalone 3-way-match / SoD UI; ~4 routes vs 22. Recurring-revenue billing now shipped (RECUR-1); late-fee auto-application + cancellation credits deferred (roadmap RECUR-2) | High |
...
| **Scheduling / dispatch (M12)** | 10 tables, 5-dimensional auto-suggest, cert-expiry auto-block, SLA auto-enforce, mobile geo clock-in | `scheduling/{page,jobs,tech}`; dispatch model (0111), availability `tech_working_hours`/`tech_absences`/`tech_certifications` (0112); `schedule-jobs.ts`, `dispatch-board.ts`, `tech-availability.ts`, `gantt.ts` | **Cert-expiry auto-block on booking DID ship** (DES-2 / SCHED-1 — `lib/api/schedule-assignments.ts` `certGate` blocks create + move when a required cert is expired against today; corrected REALITY-3). Still absent: **SLA auto-enforcement, mobile geolocation clock-in**, the 5-dimensional auto-suggest engine; `dispatch-board.ts` thin | High |
...
- **Client-facing portals are now partial** — the Quotes e-acceptance portal (`/q/[token]`) shipped (QUOTE-PORTAL-1 #397); the Invoices customer-payment portal is still designed-only. The internal client-onboarding `app/invite/[token]/client` route also exists.
...
- **Immutable/versioned snapshots are partial** — SNAP-1 balances + schedule baselines are immutable, and quote send-snapshots now exist (QUOTE-PORTAL-1 #397, frozen at send + trigger-enforced); general §0.4 #8 durability is otherwise still largely unbuilt.
...
- ~~**P1-4 — Quote client portal + e-acceptance (`/q/[token]`) + immutable send snapshots**~~ — **✅ SHIPPED (QUOTE-PORTAL-1 #397, migration 0129).** The revenue surface's close step.
...
- Scheduling: SLA auto-enforcement, mobile geolocation clock-in. (Cert-expiry auto-block on booking already shipped — DES-2; corrected REALITY-3.)
- Subcontractors: skill+territory matching; lien-deadline tracking.
- Vendors: performance auto-degrade + stored score. (Banking encryption-at-rest already shipped — SEC-2 #392; corrected REALITY-3.)
...
8. ~~**QUOTE-PORTAL-1 — `/q/[token]` e-acceptance + immutable send snapshots.**~~ **✅ SHIPPED (PR #397, migration 0129).**
...
5. **✅ RESOLVED (Jay, 2026-09-25, REALITY-3):** accounting is **integration, not replacement** — QuickBooks/Xero/Sage export (roadmap QBO-1) is the system of record; the built-in GL, period close, and bank reconciliation are **decided out** (recorded in `NEXVELON_ROADMAP.md` → "Decided out"). Original question: *confirm QuickBooks export is still "integration, not replacement" (§3), i.e. NO built-in GL/period-close/bank-rec — so those stay out of scope?*

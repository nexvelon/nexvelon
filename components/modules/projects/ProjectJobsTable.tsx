// PROJ2-4a — compact, read-only Jobs table on the project detail page. Proves
// the container model (Main Job + Change Orders) without any CRUD — full Job
// detail + editing is PROJ2-4d. Server component. Contract column is
// financials-gated (dashes without financials:edit), same rule as PROJ2-1/2-2.
          ...
          {jobs.map((j) => (
            <TableRow key={j.id}>
              ...
              <TableCell className="text-brand-charcoal text-right text-xs font-semibold tabular-nums">
                {canViewFinancials ? formatCurrency(Number(j.contract_value)) : "—"}
              </TableCell>
            </TableRow>
          ))}
        </TableBody>

...
export function useKpiData(): KpiData {
  return useContext(KpiDataContext) ?? { kpi: null, trend: null, loading: false, error: null };
}
...
export function KpiDataProvider({
  ...
  }, [active, win.valid, win.from, win.to, win.compareFrom, win.compareTo]);

  return (
    <KpiDataContext.Provider value={{ kpi, trend, loading, error }}>
      {children}

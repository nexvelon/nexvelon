...
- **Credential encryption is from day one, never retrofitted (2026-09-24, SEC-2).** Any column that stores an access secret — a gate/alarm/lockbox code, a panel or wifi password, a bank account or transit number, an integration API key — is **encrypted at rest from the migration that adds it**, using the shared `lib/crypto/credentials.ts` (application-layer AES-256-GCM). Non-negotiable key custody: the key lives ONLY in the `CREDENTIAL_ENCRYPTION_KEY` environment variable — never in a table, never in Supabase Vault, never sent to Postgres as a parameter — so a database backup contains ciphertext and no key. Decryption is server-only, behind a permission check, single-record, and NEVER included in a list/export/report/PDF; the value is masked (`••••`, §2.8) until a deliberate reveal, and a reveal writes an audit row naming the record but never the value. A field that will be searched cannot be encrypted this way — decide at design time. The first application is `vendors.account_number` (SEC-2); the schema has no gate/alarm-code columns yet, and when one is added this rule applies to it before it ever holds data.
